...
var import_auto_bind = __toESM(require_auto_bind());
var import_random_words = __toESM(require_random_words());
...
var ExtensionManager = class _ExtensionManager {
  constructor(dependencies, extensionsConfiguration) {
    ...
    this.extensions = Object.fromEntries(
      extensionsConfiguration.map((extension) => [
        _ExtensionManager.getExtensionNameByClass(extension.type),
  ...
  getExtensionInstanceByClass(extensionClass) {
    return this.extensions[_ExtensionManager.getExtensionNameByClass(extensionClass)];
  }
...
var DataColumn = class _DataColumn {
  constructor(values = []) {
    this.values = values;
  }
  sum() {
...
var DataCollection = class _DataCollection {
  constructor(data = []) {
    this.trials = data;
  }
  push(new_data) {
  ...
  top() {
    if (this.trials.length <= 1) {
      ...
      return new _DataCollection([this.trials[this.trials.length - 1]]);
    }
  }
  /**
  ...
  first(n = 1) {
    ...
    if (this.trials.length === 0) return new _DataCollection();
    if (n > this.trials.length) n = this.trials.length;
    return new _DataCollection(this.trials.slice(0, n));
  }
  /**
  ...
  last(n = 1) {
    ...
    if (this.trials.length === 0) return new _DataCollection();
    if (n > this.trials.length) n = this.trials.length;
    return new _DataCollection(this.trials.slice(this.trials.length - n, this.trials.length));
  }
  values() {
    return this.trials;
  }
  ...
  readOnly() {
    return new _DataCollection(deepCopy(this.trials));
  }
  addToAll(properties) {
  ...
  filter(filters) {
    ...
    return new _DataCollection(filtered_data);
  }
  filterCustom(fn) {
    return new _DataCollection(this.trials.filter(fn));
  }
  filterColumns(columns) {
    return new _DataCollection(
      this.trials.map(
...
var MediaAPI = class {
  ...
  getAutoPreloadList(timeline_description) {
    ...
    const traverseTimeline = (node, inheritedTrialType) => {
      ...
      } else if ((_a = node.type ?? inheritedTrialType) == null ? void 0 : _a.info) {
...
var SimulationAPI = class {
  ...
  mergeSimulationData(default_data, simulation_options) {
    return {
      ...default_data,
      ...simulation_options == null ? void 0 : simulation_options.data
    };
  }
...
function createJointPluginAPIObject(jsPsych) {
  ...
  return Object.assign(
    {},
    ...[keyboardListenerAPI, timeoutAPI, mediaAPI, simulationAPI].map((object) => (0, import_auto_bind.default)(object))
  );
}
function setSeed(seed = Math.random().toString()) {
  Math.random = (0, import_alea.default)(seed);
  return seed;
}
...
function randomWords(opts) {
  return (0, import_random_words.default)(opts);
}
function randn_bm() {
  var u = 0, v = 0;
...
var TimelineNodeStatus = ((TimelineNodeStatus2) => {
...
var ParameterObjectPathCache = class _ParameterObjectPathCache {
  constructor() {
  ...
  lookup(path) {
    ...
    const lookupPath = (path2) => {
      ...
      const lookupResult = _ParameterObjectPathCache.lookupChild(parentValue, childName);
...
var Timeline = class _Timeline extends TimelineNode {
  constructor(dependencies, description, parent) {
    super(dependencies);
    this.parent = parent;
    this.children = [];
  ...
  pause() {
    if (this.currentChild instanceof _Timeline) {
  ...
  instantiateChildNode(childDescription) {
    const newChildNode = isTimelineDescription(childDescription) ? new _Timeline(this.dependencies, childDescription, this) : new Trial(this.dependencies, childDescription, this);
  ...
  setCurrentTimelineVariablesByIndex(index) {
    ...
    this.currentTimelineVariables = {
      ...(_a = this.parent) == null ? void 0 : _a.getAllTimelineVariables(),
  ...
  getResults() {
    ...
    for (const child of this.children) {
      ...
      } else if (child instanceof _Timeline) {
  ...
  getLatestNode() {
    var _a;
    return ((_a = this.currentChild) == null ? void 0 : _a.getLatestNode()) ?? this;
  }
  getActiveTimelineByName(name) {
    var _a;
...
var JsPsych = class {
    ...
    this.dataDependencies = {
      getProgress: () => {
        ...
        return {
          ...
          trial: ((_a = this.timeline) == null ? void 0 : _a.getLatestNode().index) ?? 0
        };
      },
  ...
  getProgress() {
    ...
    return {
      ...
      current_trial_global: ((_b = this.timeline) == null ? void 0 : _b.getLatestNode().index) ?? 0,
      percent_complete: ((_c = this.timeline) == null ? void 0 : _c.getNaiveProgress()) * 100
    };
  }
  getStartTime() {
  ...
  get extensions() {
    var _a;
    return ((_a = this.extensionManager) == null ? void 0 : _a.extensions) ?? {};
  }
  async prepareDom() {
...
//# sourceMappingURL=chunk-7SS2VNBB.js.map
